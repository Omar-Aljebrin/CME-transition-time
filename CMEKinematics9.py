...
def quadratic_func_dx(x, a, b, h0):
    return 2 * a * x + b
def quadratic_func_dxdx(x, a, b, h0):
    return 2 * a 
...
afit23_q = np.full_like(xf, 2 * popt23[3])
...
# Start time (UT) provided by user
_start_ut = datetime.fromisoformat("2011-10-01T09:17:29.778")

# Convert minutes to datetime for both raw x and fitted xf
x_ut  = [ _start_ut + timedelta(minutes=float(t)) for t in x ]
xf_ut = [ _start_ut + timedelta(minutes=float(t)) for t in xf ]

# Convert velocity units: R_sun/min -> km/s
R_SUN_KM = 695700.0
RSUN_PER_MIN_TO_KMS = R_SUN_KM / 60.0  # 11595.0 km/s

vel_kms              = np.asarray(vel) * RSUN_PER_MIN_TO_KMS
yfit23_velocity_kms  = np.asarray(yfit23_velocity) * RSUN_PER_MIN_TO_KMS
softplus_velocity_kms= np.asarray(softplus_velocity) * RSUN_PER_MIN_TO_KMS
vfit23_q_kms         = np.asarray(vfit23_q) * RSUN_PER_MIN_TO_KMS

# === Plot in new units ===
plt.plot(x_ut, vel_kms, '.', label='True Data')

plt.plot(xf_ut, yfit23_velocity_kms, linestyle='-', label='Velocity (Softplus + Quadratic)')
plt.plot(xf_ut, softplus_velocity_kms, linestyle='-', label='Velocity (Softplus Only)')
plt.plot(xf_ut, vfit23_q_kms, linestyle='--', label='Velocity (Quadratic Only)')

# Extract inflection point location from softplus (in minutes), convert to UT
p2 = popt23_softplus[2]
inflect_ut = _start_ut + timedelta(minutes=float(p2))
plt.axvline(x=inflect_ut, color='gray', linestyle=':', label='Softplus inflection')

# Axis formatting for dates and km/s
ax = plt.gca()
...
# Dynamic limits
all_v = np.concatenate([vel_kms, yfit23_velocity_kms, softplus_velocity_kms, vfit23_q_kms])
...
# Set x-limits to the data range
xmin = min(min(x_ut), min(xf_ut))
xmax = max(max(x_ut), max(xf_ut))
plt.xlim(xmin, xmax)
...
plt.plot(xf, -afit23_q, color='orange', linestyle='--', label='Acceleration (Quadratic Only)')
